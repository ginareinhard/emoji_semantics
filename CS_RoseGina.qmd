...
@fricke2024semantic compared "visually similar face emojis" using an annotation system developed by @fugate2021implications. This system is based on the Facial Action Coding System (FACS) for human faces [@ekman1978facial] and was adopted for human faces by @fugate2021implications. Human-like facial features such as *eyebrows arched* and *eyes wide* are called Action Units, short AUs. For convenience, AUs are assigned numbers, allowing them to be easily referenced. As you can see in @fig-emojipairs, each emoji consists of several AUs.
...
Overall, @fig-AUpatch shows that there was indeed a preference for context-matching emojis. Importantly, participants preferred the context-matching emoji in both the AU+ condition and the AU- condition: Whether or not emoji features coincided with human facial features, did not (significantly) affect the participants' decision for one emoji or the other.

The findings thus do not provide evidence for @fricke2024semantic's hypothesis, which was based on the pictorial approach. Instead, the results suggest that visually similar emojis can convey different meanings, even when they correspond to the same human facial expressions. However, Fricke, Grosz & Scheffler [-@fricke2024semantic: 12] note that while this strongly aligns with the predictions of the lexicalist approach, it does not automatically provide clear evidence for it.
