...
Read the abstract of @fricke2024semantic's study.

a\) According to the abstract, what were the results of the experiment?
...
b\) The actual results of the experiment were different from what the authors had expected. According to @fricke2024semantic's research hypothesis (visual differences between emojis which correspond to human facial features are more semantically relevant than those that do not), which of these results would you have expected from the experiment? **(\@RoseGina, how are readers supposed to know this? Would you like them to read the abstract before attempting this question? I think that'd be a good idea but in that case you need to tell them to do so first).** <!--@Elen: Since the hypothesis and design are complex and quite difficult to understand, the question was intended to make the readers infer from the hypothesis what the outcome of the experiment would be if the hypothesis was true. The actual results of the experiment were different from what the authors expected, i.e. the abstract of the study describes different results from those asked in the quiz question. I was thinking the readers might need to understand what results the authors had expected before they conducted the experiment. The question could be confusing for the readers though. So now, as a suggestion, two questions are included as a task, let us know what you prefer / what to include. (Gina)-->
...
As specified by @fricke2024semantic (p. 8), we filter out participants who exceed the maximum age of 35 years for all following analyses. We do this by using the `filter()` function and store the result in a new data frame called `df`.
...
Before we start analysing, we should translate the labels (levels) of the categories into English. Using a combination of `mutate()` and `recode()`, we translate *männlich* to *men*, *weiblich* to *women*, and *divers* to *non-binary.*[^1]
...
[**Q1.**]{style="color:green;"} Which of these problems are likely to occur if we plot emoji understanding by gender in a barplot with unequal gender groups?
...
As mentioned above, we will visualise the relative rather than the absolute frequencies to make sure that the numbers for the different genders are comparable. In line with @fricke2024semantic (p. 9), we also exclude the three non-binary participants. To this end, we use the `filter()` function combined with the `!=` operator (see @sec-RelationalOperators).
...
[**Q2.**]{style="color:green;"} How do you interpret this plot?
...
[**Q3.**]{style="color:green;"} Which problems become apparent when checking the content of our new `context` variable using the `table()` function?
...
<br><br> [**Q6.**]{style="color:green;"} Looking at the final patchwork (@fig-AUpatch), which subplot stands out the most?
...
<br><br> [**Q7.**]{style="color:green;"} Which interpretations of the lower left plot are correct? Select all that apply.
...
<br><br> [**Q8.**]{style="color:green;"} What are plausible reasons for the striking results presented in the lower left barplot? Select all that apply.
