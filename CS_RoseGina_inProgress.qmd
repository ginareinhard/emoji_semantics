...
Face emojis are frequently used in text messages. They represent facial expressions and often make fundamental contributions to the subtext of a text message. A few studies have investigated the relationship between emojis and the emotions that they depict **(@RoseGina, such statements typically require one or more reference in academic writing. Could you please add a couple of references and send me an updated .bib file)**. However, as emojis are a relatively recent phenomenon, there is still a lot to be discovered. In this chapter, we will look into a study by @fricke2024semantic.
...
@fricke2024semantic defined two different types of emoji pairs: In the **AU+ condition**, the pairs of emojis are similar, but are assigned a different set of AUs. In the emoji pairs in the **AU- condition** are also similar, but their AUs are identical. AUs capture facial expressions by numbers and, as such, can assist linguists to accurately describe emojis. However, only expressions that can be consciously changed by humans receive number-labels: For example, the AU difference between 😃 and 😆 captures the fact that the former emoji has open eyes while the latter has closed eyes. Since humans can choose whether to open or close their eyes, this is an **AU+ pair**. If the subtle difference between emojis is not manipulable, as in 😄 and 😁, the emojis are described by identical numbers (**AU-**).

@fricke2024semantic asked whether **these two types of emojis** are treated any differently depending on the context in which they are assigned. **(\@RoseGina, which two types? So far the types referred to pairs of emojis, not single emojis. Please clarify. Also you seem to repeat the research aim at the end of the next section. Is that intended? I personally find the explanation below much clearer.)**

### The experiment {#sec-design}

::: {.callout-note title="How did the experiment work?"}
Three AU+ and three AU- emoji pairs were created (see @fig-emojipairs). Each pair was assigned two contexts, with each context corresponding to the prominent usage of one emoji, but not the other. For example, the contexts of the first pair are *happiness* and *(cheeky) laughter*. The contexts were assigned based on <https://emojipedia.org> and **a previous norming study (\@RoseGina, please include citation).**
...
These short narratives were divided up into into four experimental lists of 12 items. Each list also contained 12 filler items, so that each participant saw 24 items. The participants were then asked to help choose the emoji that best matched the context. Each participant saw each emoji pair twice. **The rate with which the context-matching emoji was chosen was measured. (\@RoseGina, this last sentence needs rephrasing).**
:::
...
Barplots from @fricke2024semantic @RoseGina**, please make clear that these plots is taken from the paper directly and add reference(s) to page number(s)**
:::

The plots in @fig-barplots show that women use emojis more often and have a more positive attitude towards emojis than men. We want to find out whether women also reported a higher level of emoji understanding than men. Our analysis will involve three steps:
...
As specified by @fricke2024semantic **(\@RoseGina, add page)**, we filter out participants who exceed the maximum age of 35 years for all following analyses. We do this by using the `filter()` function and store the result in a new data frame called `df`.
...
Before we start analysing, we should translate the categories labels (levels) into English. Using a combination of `mutate()` and `recode()`, we translate *männlich* to *men*, *weiblich* to *women*, and *divers* to *non-binary.*[^cs_rosegina_inprogress-1]

[^cs_rosegina_inprogress-1]: We decided to translate '*divers*' as 'non-binary', as this is the English term that @fricke2024semantic used in their paper.
...
The **mode** (see \@sec-Mode) of the `gender` variable in the dataset is *men*, as you can see from the output. The gender distribution is very uneven: 109 men, 47 women, and 3 non-binary people participated in the study. If we are not careful, this inbalance can lead to misleading data visualisations.
...
wrong = "Not quite. Try again.")
check_hint("Three of these problems could occur.", hint_title = "🐭 Click on the mouse for a hint.")
...
To solve this problem, we will use the same strategy as @fricke2024semantic. We will use relative rather than absolute frequencies to make sure that the numbers for the different genders are comparable. **(@RoseGina, somewhere in your chapter, you need to make clear what the Percent axis on all gender barplots actually represents. I am not convinced that all readers will understand why your bars (and the ones from the original publication!) add up to more than 100 %. Personally, I would also make that clear on the y-axis label of your own barplot.)**
...
The contexts *mild disappointment* and *moderate disappointment* have created some issues: In the `question` variable, both are described as *ist enttäuscht* ('is disappointed'). Except for their encoding in the `name` variable, these contexts appear to be identical. At this point, we have no choice but to look for additional disambiguating information in @fricke2024semantic's **analysis script (@RoseGina, please add a link or citation for the code. Also, might it be an idea to display the relevant extract of the code? Or to at least mention in which line, the information can be found?)**. The emoji 🙁 (*mild disappointment*) is coded as `N-36-L1` and ☹️ (*moderate disappointment*) as `N-37-L1`. We use this information to assign these two contexts to our `context` variable.
...
3.  Create a `ggplot` object with the context values on the *x*-axis, **percentages scaled** **(\@RoseGina, I'm not sure what you mean by this)** on the *y*-axis, and fill colours corresponding to the match values.
...
7.  Adjust colours with values from **"Set2" from the {RColorBrewer} package (see @sec-gender-understanding-vis) (\@RoseGina, see my earlier comment).**
...
<br><br> [**Q8.**]{style="color:green;"} Which interpretations of the lower left plot are correct? Select all that apply. @RoseGina**, I am struggling to get my head around two of the options for this question:**

1.  **"Only in the 😐😑 pair is the matching rate for one emoji above chance and for the other below chance." (marked as correct) --\>**

2.  **"Since matching rates in the 😐😑 plot differ a lot, AU difference seems to affect participants' preference." (marked as incorrect) --\> Am I right to think that this depends whether the statement refers to AU difference in general, across all emoji pairs, or in this specific pair? If so, you need to specify which you mean.**

**Also, please make the hint a litte bit more informative. It currently largely repeats information that the reader already gets if they got the answer wrong and I think most people will only click on the hint once they have given a wrong answer.**

```{r echo=FALSE, results="asis"}
check_question(c("The emoji 😐 was selected for its matching context considerably less often than all the other emojis.", "Only in the 😐😑 pair is the matching rate for one emoji above chance and for the other below chance."), options = c("The emoji 😐 was selected for its matching context considerably less often than all the other emojis.", "There is a major difference between the matching rates of the annoyance and happiness contexts.","Only in the 😐😑 pair is the matching rate for one emoji above chance and for the other below chance.", "Since matching rates in the 😐😑 plot differ a lot, AU difference seems to affect participants' preference."), type = "checkbox", 
random_answer_order = TRUE,
button_label = "Check answer",
...
Emojis, it turns out, contain lots of information, and there is a science behind them 🤓. While experimentally measuring why we prefer certain emojis over other ones represents a real challenge, @fricke2024semantic provides valuable insights into this fascinating area of study. As the authors shared their data and code, we were able to successfully reproduce their results, as well as create new informative figures on the basis of their data.
