...
1.  About emojis and why they are worth looking into:

Everyday, millions of people use emojis in their text messages as a means to communicate emotions. Face emojis stand in for facial expressions and thereby fundamentally contribute to the subtext of a text message. As @fricke2024semantic review, several studies have investigated the relationship between emojis and the emotions they depict. However, as emojis are a relatively recently occuring phenomenon, there is still much to be discovered. In this chapter, we will look into this study by @fricke2024semantic :

> Fricke, L., Grosz, P. G., & Scheffler, T. (2024). Semantic differences in visually similar face emojis. Language and Cognition, 1–15. \<httpsdoi:10.1017/langcog.2024.12\>.

::: {.callout-note title="How does the experiment work?"}
2.  Introducing FACS, explaining AUs and what AU+/AU- means

-   Facial Action Coding System (FACS) by \@ekman1978facial -\> facial expressions are decomposed into minimal Action Units (AUs)
-   Action Units are characteristics/ features of the emoji, such as *eyebrows arched, eyes wide* -\> emoji consists of several AUs (as shown in image emoji pairs), emoji as a puzzle with the AUs as its pieces; these units are coded as numbers
-   @fricke2024semantic use two different types of pairs of visually smilar emojis (refer to emoji_pairs.png)
    -   \[AU+\]: visual difference between emojis corresponds to a difference between AUs, e.g. *grinning face with big eyes* 😃 and *grinning squinting face* 😆 (AU difference: eyes closed, nr. 43)
    -   \[AU-\]: visual difference does not correspond to an AU difference, e.g. *grinning face with smiling eyes* 😄 and *beaming face with smiling eyes*😁 (identical AU coding)
...
3.  Briefly summarizing the pictorial @maier2023emojis and the lexicalist approach @grosz2023semantics, and their predictions for the experiment (table?)

-   pictorial:

    -   emoji as a depiction of the writer´s face in the moment of typing (Maier 2023)

    -   correspondence to human facial features as the crucial factor, if that is given

    -   semantic relevance of small visual differences between two emojis

    -   prediction: "Small visual differences that do not correspond to AU differences should be semantically less relevant than visual differences that do correspond to AU differences" (4)

-   lexicalist:

    -   emoji meanings are processed similar to/as the words that describe them (conventionalized language)

    -   “partially conventionalized meaning” of small visual differences (minimal units) regardless of whether they correspond to human faces (3)

    -   prediction: both types of difference (visual with AU and visual without AU) can create a difference in meaning between the two emojis of a pair → the context-matching emoji should always be chosen more often than the non-matching one

-   Prediction of @fricke2024semantic: Since Fricke et al. are assuming the pictorial approach, they predict that participants will choose the context-matching emoji more often in the AU+ condition than in the AU- condition (-\> higher matching rates). For the AU- pairs, the choice pattern is predicted to be more random

4.  Design

-   Two contexts for each emoji pair (each fitted to correspond to a prominent usage or meaning of one emoji but not the other, prominent usage according to Emojipedia.org and results of norming study), three \[AU+\] and three \[AU-\] emoji pairs

    → 12 contexts, 4 lexicalizations per context = 48 testing items in total, distributed over 4 experimental lists; also 12 filler/control items in each list

-   1 factor: AU difference with 2 levels: AU+/ AU; Within participants; between items

-   Dependent variable: measured rate (per condition) with which a context-matching emoji was chosen

-   Two versions of the experiment: protagonist and friend male/ protagonist and friend female

-   forced choice task: particpants were presented different contexts and were asked to help "Alex/Anna" choosing the emoji that best matches the text (6) (test_items.png)

5.  Participants

-   160 (one excluded --\> 159) participants (109 male, 47 female, 3 non-binary genders)

<!-- -->

-   German native spakers living in Germany, 18-35 years, predominently using WhatsApp on an Android phone (6 IOS, 3 "other")

-   Filler mistakes: exclusions with ≥ three mistakes → none

-   attributes:

    -   frequent WhatsApp & Emoji use

    -   Attitude towards emojis overall positive

    -   good understanding of emojis

→ participants are familiar with emojis, have a positive attitude towards them and use them frequently (9)

6.  Procedure

-   Each participant saw each emoji pair twice (in one context favoring member A and in another context favoring member B) → 6 emoji pairs, 12 contexts

-   Set of stimuli divided into two blocks, one context of a pair occured in first block and the other in the second block

-   Each block: 6 test items, 5 filler items, pseudo-randomized order; order of blocks varied between lists; in 24 of 48 stimuli the matching emoji was presented on the left side; in other 24 on the right side, order of matching/non-matching emojis varied between lists for test items

-   Post-test questions on demographics and emoji use frequency, attitude, understanding, WhatsApp use, smartphone operating system
:::
...
Among other information, @fricke2024semantic asked the participants about their gender, the frequency of emoji use on WhatsApp, their attitude towards emojis, as well as their level of emoji understanding.

They visualised the distributions of emoji use and emoji attitude by gender in the form of barplots:
...
(interpret the plots briefly?)

They did not analyse/visualise the relationship between gender and emoji understanding. In this subsection, we will create the barplot for the relationship between gender and emoji understanding step by step.

(mention steps)

### Gender frequencies / distribution of gender groups

Let's get a general overview of the gender distribution in the data. For this, we look at the variable ‘gender’ in the data set.

As you will see, the names of the different gender groups are in German. Let's translate them into English before we start our analysis.

We first have to convert 'gender' as factor so that we can look at its levels:
...
Now, we can look at what the levels of 'gender' are:
...
Next, we want to find out how well the different gender groups report to understand emojis.

Now the levels of emoji understanding:
