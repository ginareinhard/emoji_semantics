...
First, let us plan the layout of our final plot with the placeholder names for the patchworks ("p1", "p2", and "p_combined"):
...
Now, we simply fill in the names of the plots and choose some meaningful names:
...
To specify the layout, we use the `plot_layout` function. It takes the argument `guides`. By setting this argument to `"collect"`, legends that are identical across the plots within the patchwork are merged when combined.
...
@fig-AUpatch contains information on the matching rates of all emoji pairs with their contexts. However, you probably will have noticed that the plot contains two identical legends. This is the trade-off we take by using the `wrap_elements` function: We have fixated the patchworks in their state with their legends, therefore the legends cannot be merged later.
...
Create a plot with one legend for each subplot, as in @fig-alllegends. You will need to have the `patchwork` package installed and its library loaded.
