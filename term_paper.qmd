...
Before we start analysing, we should translate them into English. Using a combination of `mutate()` and `recode()`, we translate *männlich* to *men*, *weiblich* to *women*, and *divers* to *non-binary*:
...
To solve these problems, we will use the same strategy as @fricke2024semantic. We will use relative rather than absolute frequencies to make sure that the numbers for the different genders are comparable.
...
As mentioned above, we will use relative rather than absolute frequencies for our visualisation to make sure that the numbers for the different genders are comparable. We also have to exclude the very small group of three non-binary participants by filtering them out:
...
wrong = "Not quite. Try again.")
check_hint("Four of the above options are correct interpretations of the plot.", hint_title = "🐭 Click on the mouse for a hint.")
