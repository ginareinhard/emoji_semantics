...
Ideally, we would plot matching rates based on a variable that tells us exactly when the participants responded with the matching emoji. Unfortunately, @fricke2024semantic's data do not include such a variable. We will tackle this by ourselves in this section.
...
We add a variable `AU_difference` that will include all the necessary information. We do this by using a combination of `mutate`, `case_when` and `str_detect`:
...
The function `mutate` adds the column representing our variable. `str_detect` looks for a specifc string in the column we specify. In our case, the column is called `name`, and the first string we need is *AU*.

The above code command means: look for the string *AU* in the column `name`, and in all cases where you find it (`case_when`), add the string *AU+* to a new column `AU_difference`. We do this for the other conditions as well:
...
In a new data frame `data_AU`, we group the data by contexts. The command `count(match)` counts matches and non-matches for each context and stores them in the column `n`. We add the column `percent` which stores the rounded percentage of matches and non-matches for each context-pair:
...
5.  Adjust colours with values from "Set2" from the RColorBrewer package (see @sec-gender-understanding-vis).
...
9.  Plots are left-aligned by default. Since we want the emojis to be displayed on top of their corresponding context bars, we move the title to the center of the plot. We also remove the title of the legend because the *match* and *no match* values are self-explanatory.
...
So far, we have only used built-in R functions, but have not defined our own functions. Functions are reusable code snippets that perform specific tasks. You have already learned about built-in R functions in <https://elenlefoll.github.io/RstatsTextbook/7_VariablesFunctions.html> **7.4 Using built-in R functions**. However, for highly specific queries that are applied several times it makes sense to define a function yourself. As a rule of thumb, whenever code seems redundant (you may find yourself copying and pasting a lot), it is best to define a function for that task.
...
2.  We then construct the **function body** and enclose it in curly braces. The **function body** tells the function what it is meant to do when called upon.
...
3.  Our function is called `plot_AU_matches` because what it does is plotting AU matches.
