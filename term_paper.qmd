...
@fricke2024semantic have made their data and their analysis code publicly available. You can access it at <https://osf.io/k2t9p/>. There, the data is contained in the file `raw_data.csv`.
...
@fricke2024semantic compared visually face similar emojis using an annotation system developed by @fugate2021implications. This system is based on the Facial Action Coding System (FACS) for human faces invented by @ekman1978facial. @fricke2024semantic assigned numbers to human-like facial features such as *eyebrows arched* and *eyes wide*. These numbers are called Action Units, short AUs. As you can see in @fig-emojipairs, each emoji consists of several AUs:

![Emoji pairs and their contexts [@fricke2024semantic: 5]](images/emoji_pairs.png){#fig-emojipairs fig-align="center" width="700"}

@fricke2024semantic defined two different types of emoji pairs: In the **AU+ condition**, the visual difference between emojis corresponded to a difference between AUs. In the **AU- condition**, the visual difference did not correspond to an AU difference. As AUs capture facial expressions by numbers, they assist in accurately describing emojis. However, only expressions that can be consciously changed by humans receive number-labels: For example, the AU difference between 😃 and 😆 captures that the former emoji has open eyes while the latter has closed eyes. Since humans can choose whether to open or close their eyes, this is an AU+ pair. If the subtle difference between emojis is not manipulable, as in 😄 and 😁, the emojis are described by identical numbers (AU-).

@fricke2024semantic's asked whether these two types of emojis are treated any differently when being assigned to a context.

### The experiment {#sec-design}

::: {.callout-note title="How did the experiment work?"}
Three AU+ and three AU- emoji pairs were created (see @fig-emojipairs). Each pair was assigned two contexts, with each context corresponding to the prominent meaning of one emoji but not the other. For example, the contexts of the first pair are *happiness* and *(cheeky) laughter*. The contexts were assigned based on <https://emojipedia.org> and a previous norming study.

Four single-sentence narratives were created for each of the contexts. For an example, see @fig-testitems, translated from German below [taken from @fricke2024semantic: 6]:
...
![Example of a test item in @fricke2024semantic's experiment [@fricke2024semantic: 6]](images/test_items.png){#fig-testitems fig-align="center" width="500"}
...
@fricke2024semantic asked participants about their gender, their attitude towards emojis, how often they use emojis on WhatsApp and how well they think they understand emojis. @fricke2024semantic visualised the distribution of male and female gender for emoji use and emoji attitude as barplots:
...
As specified by @fricke2024semantic, we filter out participants who exceed the maximum age of 35 years for all following analyses. We do this by using the function `filter()` and store the result in a new data frame called `df`:
...
The relevant variable in the data set is called `gender`. However, you will see that the names of the gender groups are in German. To figure out what the labels of the different gender groups are, we use the `table()` function:
...
Before we start analysing, we should translate them into English. Using a combination of `mutate()` and `recode()`, we translate *männlich* to *men*, *weiblich* to *women*, and *divers* to *non-binary*[^1]:

[^1]: We decided to translate 'diverse' as 'non-binary' as this is the term @fricke2024semantic used in their paper as the translation for 'diverse'.
...
check_question(c("It may appear as if men have a great emoji understanding simply because of their group size.", "It may appear as if non-binary people have a low emoji understanding simply because of their group size.", "The differences in emoji understanding between gender groups may look bigger than they actually are.", "The differences in emoji understanding within gender groups may look smaller than they actually are.", "The differences in emoji understanding between gender groups may look bigger than they actually are."), options = c("It may appear as if men have a great emoji understanding simply because of their group size.", "The differences in emoji understanding between gender groups may look bigger than they actually are.", "The differences in emoji understanding within gender groups may look smaller than they actually are.", "It may appear as if non-binary people have a low emoji understanding simply because of their group size.", "There will be no problems if we use ggplot."), type = "checkbox",
random_answer_order = TRUE,
button_label = "Check answer",
...
To solve this problem, we will use the same strategy as @fricke2024semantic. We will use relative rather than absolute frequencies to make sure that the numbers for the different genders are comparable.
...
The variable we are interested in is called `emoji_understanding`. Just like with `gender`, we first have to do some data wrangling. We use the `table()` function to get the labels:

```{r table-understanding}

table(df$emoji_understanding)
...
Finally, our frequency table includes the percentages:

![The gender_understanding_count object (including percentages) as visualised using the View() function in RStudio](images/gender_understanding_count2.png){#fig-gender_understanding_count2 fig-align="center" width="509"}

From the percentages, we can already see that non-binary participants reported either a *rather good* or *good* understanding of emojis. A higher percentage of women (44.7%) reported a *very good* emoji understanding compared to men (38.5%). But let's create our barplot to see the distribution more clearly.

### Data visualisation {#sec-gender-understanding-vis}

As mentioned above, we will visualise the relative rather than the absolute frequencies to make sure that the numbers for the different genders are comparable. In line with @fricke2024semantic, we also have to exclude the very small group of three non-binary participants by filtering them out:
...
We use `ggplot()` to create a barplot with `emoji_understanding` on the x-axis and the relative frequencies on the y-axis. The bars are coloured according to `gender`. To make our plot more meaningful, we add a title and labels. We also remove the white space between the bottom of the bars and the x-axis and change the colours manually to make it look nicer. The hexadecimal color values chosen here are from the colour-blind friendly palette "Set2" from the package [{RColorBrewer}](https://cran.r-project.org/web/packages/RColorBrewer/index.html) [@neuwirth2022package]. Since we only need two colours, we can easily insert them manually without having to install an additional package.
...
check_question(c("Proportionally more women than men reported a very good emoji understanding.", "Proportionally more women than men reported a moderate emoji understanding.", "The bars for both genders are highest in the good to very good range."), options = c("Proportionally more women than men reported a very good emoji understanding.", "Proportionally more women than men reported a moderate emoji understanding.", "Women reported a lower level of emoji understanding than men.", "The bars for both genders are highest in the good to very good range."), type = "checkbox",
random_answer_order = TRUE,
button_label = "Check answer",
right = "That's right! While proportionally more women than men reported a very good emoji understanding, a relatively large number of men also stated that they understood emojis very well. The bars for both genders are highest in the good to very good range. There are also more women than men in the moderate and rather good category.",
wrong = "Not quite. Try again.")
check_hint("Three of the above options are correct interpretations of the plot.", hint_title = "🐭 Click on the mouse for a hint.")
...
check_question(c("All contexts have 159 occurrences, except for mild disappointment which occurs 318 times.", "There are fewer contexts in the output than we entered."), options = c("All contexts have 159 occurrences, except for mild disappointment which occurs 318 times.", "There are fewer contexts in the output than we entered.", "The context descriptions were not correctly assigned in the case of matching strings.", "There are too many occurrences of matches per context than can be reasonably assumed.", "There are more contexts in the output than we entered."), type = "checkbox", 
random_answer_order = TRUE,
button_label = "Check answer",
...
![The first 14 columns of the data frame data_AU as visualised using the View() function in RStudio](images/context_percentages.png){#fig-data-percentages fig-align="center" width="400"}
...
6.  The `expand()` function makes the bars expand vertically, thereby removing white space between the bars and the x-axis.
7.  Adjust colours with values from "Set2" from the {RColorBrewer} package (see @sec-gender-understanding-vis).
8.  Annotate the percentages of matching rates by adding them as text and placing them inside the plot, in the middle of the according bar.
9.  Add labels: *context* on the x-axis, *percent* on the y-axis, and the corresponding emojis as the title.
10. Add a theme, in this case `theme_classic()`.
11. Plots are left-aligned by default. Since we want the emojis to be displayed on top of their corresponding context bars, we move the title to the center of the plot. We also remove the title of the legend because the *match* and *no match* values are self-explanatory.
...
![Plot of individual emoji pairs that compares AU conditions [@fricke2024semantic: 11]](images/Fricke_AUplot.png){#fig-AUFricke width="700"}

First, let us plan the layout of our final plot with some placeholder names. In {patchwork}, plots are combined by `|` (horizontally), `/` (vertically), or `+` (generally).
...
The final plot will have two columns and three rows: In both `column1` and `column2`, three plots are stacked on top of each other. These are the plots of the AU+ and the AU- condition, respectively. We then put these patchworks next to each other for comparison (`columns_combined`).
...
To specify the layout, we use the `plot_layout` function which takes the argument `guides`. By setting this argument to `"collect"`, legends that are identical across the plots within the patchwork are merged when combined. We place the legends at the bottom.
...
@fig-AUpatch contains information on the matching rates of all emoji pairs with their contexts. It does not look exactly the same as the plot by @fricke2024semantic (see @fig-AUFricke) but it contains the same information. Which look do you prefer?

::: {.callout-note title="Alternative ways of dealing with the legend" collapse="true"}
You probably will have noticed that @fig-AUpatch contains two identical legends. This is the trade-off we take by using the `wrap_elements` function: We have fixated the patchworks in their state with their legends, therefore the legends cannot be merged later. There are a couple of other options that will produce different outcomes, however, none is going to be perfect: Since we do not want to delete the legend completely, we could keep the legends of all six plots, as in @fig-alllegends. Another option would be to keep the legend of one block and delete the other. However, as you can see in @fig-onelegend, this makes the bars take up the space of the legend, and the bars in one block become wider than in the other one.

::: {#fig-AU-plot-options layout-ncol="2"}
![The plot with six legend.](images/AU_plot_six_legends.png){#fig-alllegends fig-align="center" width="550"}

![The plot with one legend.](images/AU_plot_one_legend.png){#fig-onelegend fig-align="center" width="550"}
...
```{r echo=FALSE, results="asis"}
check_question(c("The emoji 😐 was selected for its matching context significantly less often than all the other emojis.", "Only in the 😐😑 pair is the matching rate for one emoji above chance and for the other below chance."), options = c("The emoji 😐 was selected for its matching context significantly less often than all the other emojis.", "There is a major difference between the matching rates of the annoyance and happiness contexts.","Only in the 😐😑 pair is the matching rate for one emoji above chance and for the other below chance.", "Since matching rates in the 😐😑 plot differ a lot, AU difference seems to affect participants' preference."), type = "checkbox", 
random_answer_order = TRUE,
button_label = "Check answer",
...
You are now a pro in handling barplots! You can build, customise, arrange, and interpret them. Barplots are powerful for visualising categorical data, offering a straightforward way to compare frequencies and make patterns apparent. However, they do have their limitations. For instance, they are not ideal for displaying continuous data. Building and assembling plots can be quite fiddly and it can take some trial-and-error to make the plot look like what you have imagined. But there is a solution for (almost) everything and hopefully, the beautiful plot you create in the process will be worth the effort.

This chapter's analysis revealed gender-specific differences in emoji understanding, potentially indicating a gender confidence gap between men and women. Both genders, however, reported understanding emojis well overall. The visualisations have been adjusted for the gender imbalance in the data, demonstrating the importance of accounting for differences in sample sizes.

In this chapter, we have created an informative picture that answers the experiment’s research question by combining multiple plots. The question whether Action Unit (AU) differences are critical for emoji preference was answered in the negative. However, we have made several other discoveries along the way: Similarly to @fricke2024semantic, we have found that small changes of emojis’ facial features do affect choice patterns.

Emojis, it turns out, contain lots of information, and there is a science behind them 🤓. While measuring why we prefer certain emojis over other ones can be challenging, @fricke2024semantic's study and their analysis, as well as our reanalysis, have provided insights into this fascinating area of study.
