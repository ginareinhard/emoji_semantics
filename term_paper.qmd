...
In line with the research question of @fricke2024semantic, we want to compare the matching rates of emojis and contexts in the AU+ condition with the matching rates in the AU- condition. Our goal is to create a plot that looks similar to the one by @fricke2024semantic:

![Plot of individual emoji pairs that compares AU conditions [by @fricke2024semantic, p.11]](images/Fricke_AUplot.png){#fig-AUFricke width="800"}

First, let us plan the layout of our final plot with the placeholder names for the patchworks (`p1`, `p2`, and `p_combined`). In `patchwork`, plots are combined by `|` (horizontally), `/` (vertically), or `+` (generally).
...
It does not look exactly the same as the plot by @fricke2024semantic, but it contains the same information. Which look do you prefer?

Building plots and assembling them can be quite fiddly. You may get lost in details and may need to accept some trade-offs. It can take some time and trial-and-error to make the final plot look like what you have imagined. However, there is a solution for (almost) anything and hopefully, the beautiful plot you create in the process will make up for the trouble.
