...
Before we dive into their study, we need to introduce the Facial Action Coding System (FACS) which was invented by @ekman1978facial. Based on this system, @fugate2021implications developed a face emoji annotation system: They assigned numbers to features such as *eyebrows arched* and *eyes wide*. These numbers are called Action Units, short AUs. As you can see in \[image emoji pairs\], each emoji consists of several AUs. This system makes it possible to describe emojis systematically and unambiguously.

@fricke2024semantic compare visually similar emojis, but they distinguish two different types of emoji pairs: In the \[AU+\] condition, the visual difference between emojis corresponds to a difference between AUs, e.g. *grinning face with big eyes* 😃 and *grinning squinting face* 😆 (AU difference: eyes closed, nr. 43). In the \[AU-\] condition, the visual difference does not correspond to an AU difference, e.g. *grinning face with smiling eyes* 😄 and *beaming face with smiling eyes*😁 (identical AU coding).

::: {.callout-note title="How does the experiment work?"}
3.  Briefly summarizing the pictorial @maier2023emojis and the lexicalist approach @grosz2023semantics, and their predictions for the experiment (table? is it even relevant?)
...
Following the pictorial approach, @fricke2024semantic predict that participants will choose the context-matching emoji more often in the \[AU+\] condition than in the \[AU-\] condition (-\> higher matching rates). For the \[AU-\] pairs, choice patterns are predicted to be more randomly distributed.
...
Among other information, @fricke2024semantic asked the participants about their gender, how often they use emojis on WhatsApp, what their attitude towards emojis is, and how well they think they understand emojis. They visualised the distribution of emoji use and emoji attitude for male and female gender in the form of bar plots:
...
What they have not visualised is the distribution of emoji understanding by gender. In this subsection, we will create the bar plot for emoji understanding by gender step by step.

(mention steps)

### Gender frequencies / distribution of gender groups

Let's get a general overview: How many men, women, and non-binary people participated in the study?

The relevant variable in the data set is called `gender`. However, you will see that the names of the different gender groups are in German. Before we start analysing, we should translate them into English.

To figure out what the the labels of the different genders are, we use the `levels()` function. This function applies to factors, so we first need to convert `gender` as a factor:
