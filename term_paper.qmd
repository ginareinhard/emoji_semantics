...
@fricke2024semantic defined two different types of emoji pairs: In the **AU+ condition**, the visual difference between emojis corresponded to a difference between AUs. In the **AU- condition**, the visual difference did not correspond to an AU difference. As AUs are facial expressions captured by numbers, they help in accurately describing emojis. However, only expressions that can be consciously changed by humans receive number-labels: For example, the AU difference between 😃 and 😆 captures that the former emoji has open eyes while the latter has closed eyes. Therefore, this is an AU+ pair. If the subtle difference between emojis is not manipulable, as in 😄 and 😁, the emojis are described by identical numbers (AU-).

@fricke2024semantic's ask whether these two types of emojis are treated any differently when being assigned to a context.
...
The relevant variable in the data set is called `gender`. However, you will see that the names of the different gender groups are in German. To figure out what the the labels of the different gender groups are, we use the `table()` function:
