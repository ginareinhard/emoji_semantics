...
To make our plot more meaningful, we add a title and labels. We also change the colours manually to make it look nicer. The hexadecimal color values chosen here are from the colour-blind friendly palette "Set2" from the package "RColorBrewer". Since we only need two colours, we can insert them manually without having to install an additional package.
