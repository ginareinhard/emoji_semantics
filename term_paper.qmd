...
To specify the layout, we use the `plot_layout` function. It takes the argument `guides`. By setting this argument to `"collect"`, legends that are identical across the plots within the patchwork are merged when combined. We place the legends at the bottom.
...
By combining multiple plots, we have created an informative picture that answers the experiment’s research question. The question whether Action Unit (AU) differences are critical for emoji preference was answered in the negative. However, we have made several other discoveries along the way, and have found that small changes of emojis’ facial features do affect choice patterns.
