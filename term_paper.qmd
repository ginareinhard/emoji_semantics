...
>
>     *I just learned that my cousin´s dog has her own advent 
>     calender.*
>     
>      Alex is amused.
...
This graph contains information on the matching rates of all emoji pairs with their contexts. However, you will probably notice that the graph contains two identical legends. This is the trade-off we take by using the `wrap_elements` function: We have fixated the patchworks in their state with their legends, therefore the legends cannot be merged later.
...
The emoji pair *mild irritation* 😐 - *annoyance* 😑 sets itself apart from the rest: There is a considerable difference between the matching rates. Perhaps counter intuitively, this means that participants opted for the same emoji in both contexts.

Apparently, 66.67 % (two thirds) of participants favoured the non matching emoji in the *mild irritation*-context, so they chose 😑 in the context "is slightly irritated". As an example, one of the single-sentence-narratives in this context was "a malfunctioning wifi router". Which emoji would you choose in this context, 😐 or 😑? 
...
Overall, the plot shows there was indeed a preference for context-matching emojis. However, the findings do not support the pictorial approach adopted by @fricke2024semantic: In the experiment, whether or not emoji features coincided with human facial features, did not (significantly) affect the participants´ decision for one emoji or the other. 
