...
>
>     *I just learned that my cousin´s dog has his own advent 
>     calender.*
>     
>      Alex is amused.
>     Which of the emojis matches the message better?
>     😄😁
>
>
...
>
>     Alex is overjoyed.
>     Which of the emojis matches the messsage better?
>     😄😁
...
1\)  Which of these problems are likely to occur if you plot emoji understanding by gender in a barplot with unequal group sizes?

```{r echo=FALSE, results="asis"}

check_question(c("It may appear as if men have a greater emoji understanding simply because of their group size.", "It may appear as if non-binary people have a lower emoji understanding simply because of their group size.", "The differences in emoji understanding between gender groups may look bigger than they actually are."), options = c("It may appear as if men have a greater emoji understanding simply because of their group size.", "The differences in emoji understanding between gender groups may look bigger than they actually are.", "The differences in emoji understanding within gender groups may look smaller than they actually are.", "It may appear as if non-binary people have a lower emoji understanding simply because of their group size.", "There will be no problems if we use ggplot."), type = "checkbox",
random_answer_order = TRUE,
button_label = "Check answer",
right = "That´s right! Unequal group sizes can exaggerate the differences by showing larger totals for larger groups and smaller totals for smaller groups. <br><br>Can you think of ways to address these issues?",
wrong = "Not quite. Here’s a hint: ")

```
:::

To solve this problem, we will, like @fricke2024semantic, use relative rather than absolute frequencies and exclude the very small group of three non-binary participants for our visualisation.
...
2\) Which problem becomes apparent when checking the output via the table? Select the one that describes the issue most accurately.
...
Functions in R are defined using the keyword `function`. After this keyword, we write a list of **parameters** in parantheses. **Parameters** act as placeholders for the function's actual arguments. 
We then construct the **function body** and enclose it in curly braces. The **function body** tells the function what it is meant to do when called upon. 
...
2\) In which context did participants choose the matching emoji most often?
...
<br><br> 3\) How can a small difference (such as < 2%) between matching rates within pairs be interpreted?

```{r echo=FALSE, results="asis"}
check_question("Within these pairs, a very similar amount of participants chose the matching emoji.", options = c("Within these pairs, the same amount of participants chose the matching emoji.", "Within these pairs, participants struggled the most to decide for one emoji over the other.", "Within these pairs, the least amount of participants chose the matching emoji in these pairs.", "Within these pairs, the distribution of matching rates is almost 50/50: half of the partcipants preferred the one emoji and half of them preferred the other."), type = "radio", 
random_answer_order = TRUE,
button_label = "Check answer",
...
<br><br> 3\) Looking at the final patchwork (@fig-AUpatch), which "sub-plot" stands out from the rest?
...
<br><br> 4) Which interpretations of this finding are correct? Select all that apply.

```{r echo=FALSE, results="asis"}
check_question("The emoji 😐 was chosen significantly less for its matching context than all the other emojis.", options = c("The emoji 😐 was chosen significantly less for its matching context than all the other emojis.", "There is a major difference between matching rates of the contexts annoyance and happiness.","Only in the 😐😑 pair is the matching rate for one emoji above chance and the matching rate for the other emoji below chance.", "Since the matching rates differ so much in the lower left plot, AU+ does seem to affect participants' preference."), type = "checkbox", 
random_answer_order = TRUE,
button_label = "Check answer",
...
<br><br> 5)  Which of the following reasons are likely to have caused this variance? Select all that apply.

```{r echo=FALSE, results="asis"}

check_question("Participants perceived the stories of the context slightly irritated as more annoying than was expected by the authors.", "The stories created for being slightly irritated and being annoyed were too similar to each other.", options = c("Participants perceived the stories of the context slightly irritated as more annoying than was expected by the authors.", "The stories created for being slightly irritated and being annoyed triggered a similar reaction.", "Unlike what was anticipated, most participants use the emojis 😐 and 😑 in very different contexts.", "The participants did not realize there was a difference between 😐 and 😑."), type = "checkbox",
random_answer_order = TRUE,
button_label = "Check answer",
...
wrong = "Not quite.", alignment = "vertical")
