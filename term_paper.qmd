...
The variable we work with is called `emoji_understanding`. Just like with `gender`, we first have to do some data wrangling. Again, we convert `emoji_understanding` as a factor to get its levels:
...
Finally, we print our frequency table, selecting the columns `gender`, `emoji_understanding`, `n`, and `percentage` by applying `select()`:
