...
[**Q1.**]{style="color:green;"} What problems are likely to occur if you plot emoji understanding by gender in a barplot with unequal group sizes?

```{r echo=FALSE, results="asis"}

check_question("", options = c("It will appear as if men have a greater emoji understanding simply because of their group size.", "The differences in emoji understanding between gender groups will look smaller than they actually are.", "The differences in emoji understanding within gender groups will look bigger than they actually are.", "The bars for the non-binary group will appear huge.", "There will be no problems if we use ggplot."), type = "radio", 
random_answer_order = TRUE,
button_label = "Check answer", q_id = 5,
right = "That´s right! Unequal group sizes can exaggerate differences in understanding by showing larger totals for bigger groups.",
wrong = "Not quite, try again.")

```

[**Q2.**]{style="color:green;"} How could you address these issues?

```{r echo=FALSE, results="asis"}

check_question("", options = c("", "", "", "", "", ""), type = "radio", 
random_answer_order = TRUE,
button_label = "Check answer", q_id = 5,
...
This is likely to skew our visualisation. To solve this problem, we will, like @fricke2024semantic, use relative rather than absolute frequencies and exclude the very small group of three non-binary participants for our visualisation.
...
There are various ways to insert emojis in R. The easiest is to use the emoji keyboard. To open it on MacOS, use the keyboard shortcut `Crtl + Cmd + Space` or `fn + e` and on Windows `Windows logo key + . (period)`. The emoji keyboard is also available in RStudio, if you go to the "Edit" drop-down menu and click on "Emojis & Symbols’. Alternatively, there are emoji libraries for R, for example "emo(ji)" developed by @wickham2024emoji.

As we want to display emojis within plots, we need to pay even more attention to graphics. Emojis as part of plots created by ggplot, for example, cannot be displayed just like that. Additional problems can occur when rendering a Quarto or R Markdown document to HTML.

If displaying emojis as part of plots in RStudio does not work for you, you will need to use AGG or Cairo as a backend in RStudio. To do this, head to the "Tools" drop-down menu and click on "Global Options". Then go to the "Graphics" tab and select the "AGG" or "Cairo" option (see @fig-agg). \[explain AGG/Cairo\]
...
For rendering the output from .qmd (or Markdown?) to HTML (or pdf?) you can either:
...
[**Q3.**]{style="color:green;"} Looking at the final patchwork, which "sub-plot" shows the most significant finding?
...
[**Q4.**]{style="color:green;"} Which interpretation of this finding is most accurate?
...
The answer is no, seemingly not. AU-difference does not seem to be critical to the choice of an emoji in a context. In the plot, the matching emoji was "generally preferred with matching rates above chance level" in the AU+ and the AU- condition (exception: mild irritation). However, we also see that minor visual differences between emojis do affect the understanding and choice of emojis in different contexts. 
...
"each emoji is preferred over a visually similar emoji in a context that represents a prominent aspect of its meaning, as determined on the basis of the emoji’s Emojipedia.org entry and a norming experiment. Thus, subtle differences in emojis that otherwise look alike lead to differences in meaning."
