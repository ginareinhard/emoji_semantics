...
Women seem to use emojis more often and have a more positive attitude towards emojis than men. We want to find out whether women also reported a higher level of emoji understanding than men. Our analysis will involve 3 steps:
...
3.) visualising the frequencies in a bar chart similar to the charts above
...
Now that our values look correct, we can count the frequencies for the different gender groups within `emoji_understanding`. We create a new data frame ‘gender_emoji_count’ and again keep only the unique submission_id as above. We group the data by gender and count the frequencies for the different genders within the `emoji_understanding` factor:
...
<!-- insert screenshot of head()/table? in order to not confuse readers -->

Since the sizes of the gender groups are very unequal, we add relative frequencies to make the numbers comparable. We calculate them using the formula `percentage = (n/total) * 100`, where we define `total` as `sum(n)` (in other words: the sum of the absolute numbers within each gender):
...
### Visualising gender vs. emoji understanding
...
We use ggplot() to create a barplot for emoji understanding by gender (male and female):
...
(adjust color manually to colors of @fricke2024semantic)
...
geom_text(aes(label = percent), position = position_stack(vjust = 0.5)) + #5.
labs (x= "context", y = "percent", title = "😯 😲") + #6.
theme_bw() + #7.
theme(plot.title = element_text(hjust = 0.5)) #8
```
...
3.  Display the plot as a bar chart. By default, `geom_bar` counts how many times matches/ no matches occurred. However, as our data already includes the counts for each group (column `n`), we use `geom_col` to use the data as is.
4.  Order contexts so they match the emojis
5.  Add text to the plot (percent values) and place it inside the bars
6.  Add labels: context on the x axis, percent on the y axis, emojis as title
7.  Add an appropiate and pratical theme (theme_bw)
8.  Plots are left-aligned by default. Since we want the emojis to be displayed on top of their matching context bars, move the title to the center of the plot.
...
On our case, the process of defining a function is straightforward: We simply repeat the code we just wrote, but with the general argument names ("emojis" and "contexts").
...
```{r echo=FALSE, results="asis"}
check_question("😐 was chosen significantly less in its matching context than all the other emojis.", options = c("😐 was chosen significantly less in its matching context than all the other emojis.", "upper right", "middle left", "middle right", "lower left", "lower right"), type = "radio", 
random_answer_order = TRUE,
