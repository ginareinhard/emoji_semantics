...
Four single-sentence-narratives were created for each of the contexts. See @fig-testitems, and its following translation [taken from @fricke2024semantic, p.6], for an example:
...
The context illustrating situations were presented to participants, and participants were asked to help choosing the fitting emoji. Each participant saw each emoji pair twice. The rate with which the context-matching emoji was chosen was measured.
:::
...
Ideally, we would plot matching rates based on a column that tells us exactly when the participants responded with the matching emoji. Unfortunately, @fricke2024semantic´s data does not include such a column. We will tackle this by ourselves in this section.

### Preparing the data {#sec-au-data-preparation}

First, we need a column that includes the experimental conditions of each trial. The experimental conditions include whether the visual difference between the presented emojis correponded to an AU-difference (AU+) or not (AU-), or whether a filler was presented.
...
We will create another column called `context` to which we will add the context descriptions used in @fig-emojipairs. Again, we combine `mutate`, `case_when` and `str_detect`: In the column `question`, we look for a context-characteristic strings, and add the context descriptions in case of a match. We check the output with `table()`.
...
The data is grouped by contexts. Matches and non-matches are counted for each context and stored in a column `n`. We add a column `percent` which stores the percentage of matches and non-matches for each context-pair.
...
3.  Display the plot as a barplot. By default, `geom_bar` counts how many times matches/ no matches occurred. However, as we have already calculated the values of metric we want to use (column `percent`), we use `geom_col` to use the data as is.
...
So far, we have only used built-in R functions, but have not defined any functions ourselves. Functions are reusable blocks of code that perform specific tasks. You have already learned about built-in R functions in <https://elenlefoll.github.io/RstatsTextbook/7_VariablesFunctions.html> **7.4 Using built-in R functions**. However, it makes sense to define a function for highly specific queries that are applied multiple times.
...
The answer is no, seemingly not. AU-difference does not seem to be critical to the choice of an emoji in a context. As @fig-AUpatch shows, the matching emoji was "generally preferred with matching rates above chance level" [@fricke2024semantic] both in the AU+ and in the AU- condition.
...
Through the following quiz questions, we will further interpret the graph (@fig-AUpatch). The correct answers will reveal how the information displayed can be made sense of.
...
random_answer_order = TRUE,
button_label = "Check answer",
right = "That's right! The great majority of particpants (86.79% and 88.68%) decided for the matching emoji in the contexts happiness (😃) and (cheeky) laughter (😆).",
wrong = "Not quite, try again.")
...
check_hint("A small difference implies that match/no match ratios are very similar across contexts. For example, this is the case in the subplot *happiness* 😃 - *(cheeky) laughter* 😆.", hint_title = "🐭 Click on the mouse for a hint.")
```

<br><br> [**Q5.**]{style="color:green;"} Looking at the final patchwork (@fig-AUpatch), which "sub-plot" stands out from the rest?
...
random_answer_order = TRUE,
button_label = "Check answer",
right = "That's right! Apparently, two thirds (66.67 %) of participants favoured the non matching emoji in the *mild irritation*-context: Instead of 😐, they chose 😑. One of the single-sentence-narratives in this context was 'a malfunctioning wifi router'. Which emoji would you choose in this context, 😐 or 😑?" ,
