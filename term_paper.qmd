...
In this chapter, you will learn how to: <!-- Vorschläge -->

-   overview and explore the data of a study
-   access specific data you are interested in
-   analyse and interpret frequency
-   visualise data via barplots
-   build and interpret a complex plot

In this chapter, we will work with data of the study by @fricke2024semantic. @fricke2024semantic have made their data and their analysis code available to the public. You can access it at <https://osf.io/k2t9p/>. The data is contained in the file `raw_data.csv`.
...
@fricke2024semantic compare visually similar emojis using a face emoji annotation system developed by @fugate2021implications. This annotation system is based on the Facial Action Coding System (FACS) for human faces invented by @ekman1978facial. @fricke2024semantic assign numbers to features such as *eyebrows arched* and *eyes wide*. These numbers are called Action Units, short AUs. As you can see in @fig-emojipairs, each emoji consists of several AUs:
...
```
@fricke2024semantic predict, in line with the pictorial approach by @maier2023emojis, that visual differences between emojis which do not correspond to human facial features (AU-) are less semantically relevant compared to those that do correspond to such features (AU+).
...
Four single-sentence-narratives were created for each of the contexts. For an example, see @fig-testitems, translated in the following [taken from @fricke2024semantic, p.6]:
...
>     lottery.*
>
>     Alex is overjoyed. Which of the emojis matches the messsage better? 😄😁

These stories were then divided up into into four lists of 12 items. Each list also contained 12 filler items, so that each participant saw 24 items.

The context-illustrating narratives were presented to the participants, and the participants were asked to help choosing the fitting emoji. Each participant saw each emoji pair twice. The rate with which the context-matching emoji was chosen was measured.
:::
...
The lexicalist approach predicts that it does not matter whether visual (AU) differences correspond to facial characteristics or not: For both types of pairs, the context-matching emoji should be preferred over the non-matching one. <!-- erstmal hierhin verschoben, weil von den Kontexten vor der blauen Box noch nicht die Rede war -->
...
We import the data using the `here` function, as explained in <https://elenlefoll.github.io/RstatsTextbook/6_ImpoRtingData.html> **6.5 Importing data from a .csv file**.
...
[**Q1.**]{style="color:green;"} Which of these problems are likely to occur if you plot emoji understanding by gender in a barplot with unequal group sizes?
...
[**Q2.**]{style="color:green;"} Which problem becomes apparent when checking the output via the table? Select the one that describes the issue most accurately.
...
If displaying emojis as part of plots in RStudio does not work for you, you will need to use AGG or Cairo as a backend in RStudio. To do this, head to the "Tools" drop-down menu and click on "Global Options". Then go to the "Graphics" tab and select the "AGG" or "Cairo" option (see @fig-agg). AGG stands for Anti-Grain Geometry \[explain AGG/Cairo\]
...
To correctly render the output from a Quarto document to HTML, you can use the "ragg" library developed by @pedersen2024ragg. This library provides graphic devices based on AGG and includes advanced text rendering, with support for emojis.

"Use ragg with knitr: knitr supports png output from ragg by setting `dev = "ragg_png"` in the chunk settings or globally with `knitr::opts_chunk$set(dev = "ragg_png")`." (<https://ragg.r-lib.org/index.html>)
:::
...
[**Q3.**]{style="color:green;"} In which context did participants choose the matching emoji most often?
...
<br><br> [**Q4.**]{style="color:green;"} How can a small difference (such as \< 2%) between matching rates within pairs be interpreted?
...
<br><br> [**Q5.**]{style="color:green;"} Looking at the final patchwork (@fig-AUpatch), which "sub-plot" stands out most from the rest?
...
<br><br> [**Q6.**]{style="color:green;"} Which interpretations of this finding are correct? Select all that apply.
...
<br><br> [**Q7.**]{style="color:green;"} Which of the following reasons are likely to have caused this variance? Select all that apply.
